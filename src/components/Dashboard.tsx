    ...
    <div className="w-full max-w-6xl mx-auto px-6 py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredEvents.length === 0 ? (
           <div className="col-span-full text-center py-20 text-gray-500 border-2 border-dashed border-gray-800 rounded-3xl">
             {viewMode === 'MY' ? "No events found. Create one or place a bet!" : "No events found."}
           </div>
        ) : (
          filteredEvents.map((event) => (
            <div key={event.id} className="bg-gray-900 border border-gray-800 p-6 rounded-2xl relative group hover:border-gray-600 transition">
              ...
              {/* Participation Badge */}
              {address && event.participants.includes(address) && (
                <span className="absolute top-4 right-4 bg-blue-500/20 text-blue-400 text-[10px] font-bold px-2 py-1 rounded border border-blue-500/30">
                  JOINED
                </span>
              )}

              {/* THE GOOD DELETE BUTTON (Creator Only) */}
              {address && event.creatorAddress === address && (
                <button 
                  onClick={(e) => handleDelete(e, event.id)}
                  className="absolute top-4 right-4 text-gray-600 hover:text-red-500 transition z-10 p-2 bg-black/20 rounded-lg hover:bg-black/50"
                  title="Delete Event"
                >
